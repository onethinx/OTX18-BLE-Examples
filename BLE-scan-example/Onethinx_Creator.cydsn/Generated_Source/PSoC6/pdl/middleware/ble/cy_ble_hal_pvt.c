...
uint32_t Cy_BLE_HAL_GetIcPackageType(void)
{
    ...
    if( ((pType == (uint8_t) CY_GPIO_PACKAGE_BGA) && (pinCount == 0u)) ||
        ((pType == (uint8_t) CY_GPIO_PACKAGE_BGA) && (pinCount == PACKAGE_116_PINS)) )
    {
    ...
    else if((pType == (uint8_t) CY_GPIO_PACKAGE_QFN) && (pinCount == PACKAGE_68_PINS))
    {
